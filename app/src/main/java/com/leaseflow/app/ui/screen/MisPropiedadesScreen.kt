...
@Composable
fun MisPropiedadesScreen(navController: NavHostController) {
    ...
    Scaffold(
        topBar = { AppTopBar(title = "Mis propiedades") },
    ) { paddingValues ->
        Column(
            modifier = Modifier.fillMaxSize().padding(paddingValues).padding(16.dp),
            verticalArrangement = Arrangement.spacedBy(12.dp),
        ) {
            Button(onClick = { navController.navigate(Routes.AgregarPropiedad) }) {
